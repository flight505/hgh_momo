...
def main():
    ...
    with st.sidebar:
        ...
        units_in_vial = st.slider("Units in vial (IU)", 1, 100, 1, 1)
        ba_water_added_to_vial = st.slider("BA water added to vial (mL)", 0.1, 10.0, 2.0, 0.1)
    ...
    with st.sidebar:
        st.write("---")
        total_vol_syringe = st.slider("Total vol syringe (mL)", 0.1, 1.0, 0.0, 0.1)
        total_tick_marks = st.slider("Total tick marks", 1, 100, 0, 1)
...
#https://media.giphy.com/media/8BuX0qwZidk8a2xQNx/giphy-downsized-large.gif
